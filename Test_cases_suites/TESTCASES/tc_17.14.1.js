...
casper.test.begin("  Notebook Description as Search Text", 5, function suite(test) {
    ...
    //Edit the notebook title so that number of search results are less
    casper.viewport(1024, 768).then(function () {
    ...
    casper.viewport(1024, 768).then(function () {
        ...
        casper.then(function () {
            this.sendKeys('#input-text-search', title);
            this.wait(6000);
            var z = casper.evaluate(function () {
                $('.icon-search').click();
            });
