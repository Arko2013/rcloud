...
casper.test.begin("Edit loaded notebook (published)", 11, function suite(test) {
    ...
    casper.viewport(1366, 768).then(function () {
        ...
        this.test.assertEquals(title,"loading...","Confirmed that notebook title is not displayed ");
        this.test.assertNotVisible({type:'css',path:'#command-prompt > textarea:nth-child(1)'},"Confirmed that cells are not visible");
    });
