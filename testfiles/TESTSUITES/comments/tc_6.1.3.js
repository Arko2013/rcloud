...
casper.test.begin(" Increment in count of comments", 7, function suite(test) {
    ...
    //enter the comment and verify increase in count
    functions.comments(casper, comment);
    casper.then(function () {
        var comment_count = this.fetchText({type: 'css', path: '#comment-count'});
        this.test.assertNotEquals(comment_count, '0', 'New comment has been entered.Hence the count of comments has increased');
    });
