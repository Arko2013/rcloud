/* 
 Author: Arko
 Description:    This is a casperjs automated test script for showing that before a history version is reverted back to, the notebook title will have a "read only" tag attached
 with it
...
casper.test.begin("read-only tag for an unreverted history version", 10, function suite(test) {
    ...
    casper.then(function () {
        ...
        this.test.assertExists({type: 'css', path: '#readonly-notebook'}, "The history version is in read-only state state");
    });
